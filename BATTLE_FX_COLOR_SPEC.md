...
**場地 Icon 美術（HUD／面板／預報共用）**

| 項目 | 定案 |
|------|------|
| 策略 | 四系各 **1 張 Sprite**；對戰 HUD、**場地效果**面板、天氣預報 overlay **共用同一套**，程式端縮放 |
| 出圖尺寸 | **128×128 px**（對齊大地圖節點 @2× 出圖習慣，見 [`詳細設計文件.md`](詳細設計文件.md) §5.2.1） |
| 圖形安全區 | 符號本體 **88～96 px** 正方形置中；四周 **16 px** 透明邊（光暈、圓角 mask 不切到圖） |
| 風格 | 低飽和、暖色訓練廳；主色對齊各系 §4.x 色票（`BattleFxColors`），**非**霓虹／戶外天氣 icon |
| Unity 匯入 | Sprite (2D and UI)、**PPU 100**、Alpha is Transparency |
| 建議路徑 | `Assets/UI/Battle/WeatherIcons/`（例：`weather_ember_hearth.png`、`weather_warm_lamplight.png`、`weather_training_mist.png`、`weather_hall_draft.png`） |

| 顯示位置 | `Image.sizeDelta`（參考） | 備註 |
|----------|---------------------------|------|
| HUD 角標（`WeatherBadgeText` 旁） | **36×36**～**40×40** | 與字級 28 並排 |
| 場地效果面板（`ActiveWeatherEffectPanel`） | **48×48** | 摘要列左側 icon |
| 天氣預報卡（`WeatherForecastCard`） | **96×96**～**112×112** | 標題上方主 icon |
...
| 2026-07-21 | 1.4 | §4.0 新增場地 Icon 美術：128×128 四系共用，HUD／面板／預報縮放 |
