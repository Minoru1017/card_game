...
| **敵方** | 溫和 AI；壓力低於港灣普通 |
| **教練** | **全程**提示：何時上御三家、何時會觸發戰技（`TutorialBattleCoachUi` 擴充或 M-1-2 專表，見 L1-2-009） |
...
**教練提示順序（建議）**：民兵上場（列陣）→ 王后上場挨刀（庇護）→ 國王在場被直擊（庭訓）。

**刻意不放**：修女、主教、城堡、騎兵、護林鹿（避免與 B 段教會線混淆）。

**程式占位**：`M12PhaseADeckCardIds`（待實裝，對照上表順序）。
...
**程式占位**：`M12PhaseBDeckCardIds`（待實裝，對照上表順序）。
...
| **內容** | 肯定 A 段御三家戰技已觸發；預告 B「教會三張搭配」加練 |
...
- **階段 B**：**須勝利**；御三家戰技以 **A+B 合計**各 ≥1 為段考通關條件（B 任務欄顯示合計進度；A 通常已滿足三項）。
...
| A／B 定案牌表 | §3.1（15 張）· §3.2（20 張）；待 `M12PhaseADeckCardIds`／`M12PhaseBDeckCardIds` |
