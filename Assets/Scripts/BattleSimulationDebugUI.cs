...
public partial class BattleSimulationDebugUI : MonoBehaviour
{
    ...
    [Tooltip("Demo 預設關閉除錯半屏；僅開發除錯時勾選。評審流程不需開啟即可完整對戰。")]
    ...
    private TextMeshProUGUI endBattleSubtitleText;
    /// <summary>結算標題區僅顯示「結果 · 回合數」一行；完整戰報見「對戰歷史」。</summary>
    private TextMeshProUGUI endBattleHeaderStatsText;
    ...
    private void SyncHandUiIfBattleAlreadyStarted()
    {
        if (battleManager == null || handArea == null) return;
        if (battleManager.GetPlayerHandCount() <= 0 && battleManager.GetEnemyHandCount() <= 0) return;
    ...
    private void Update()
    {
        ...
        else
        {
            ...
            else if (TutorialBattleCoachUi.IsActiveForCurrentBattle)
        ...
        TryHandleTutorialPlayerFieldMonsterLost();
    }
    ...
    private void EnsureTutorialBattleCoach(Transform canvasRoot)
    {
        if (!TutorialBattleCoachUi.IsActiveForCurrentBattle) return;
        TutorialBattleCoachUi coach = GetComponent<TutorialBattleCoachUi>();
        if (coach == null) coach = gameObject.AddComponent<TutorialBattleCoachUi>();
        coach.Initialize(battleManager, canvasRoot, sharedUIFont, this);
    }

    private void EnsureHarborCombatCoach(Transform canvasRoot)
    {
        if (!HarborCombatCoachUi.IsActiveForCurrentBattle) return;
        HarborCombatCoachUi coach = GetComponent<HarborCombatCoachUi>();
        if (coach == null) coach = gameObject.AddComponent<HarborCombatCoachUi>();
        coach.Initialize(battleManager, canvasRoot, sharedUIFont, this);
    }
    ...
    private void CreateDebugPanel(Transform parent)
    {
        ...
        CreateRoundInitiativeHud(parent);
    ...
    private void OnPauseGiveUpClicked()
    {
        ...
        {
            StartCoroutine(CoGiveUpWithDefeatPrompt());
            return;
        }

        StartCoroutine(CoShowGiveUpDefeatPromptThenExit());
    }

    private IEnumerator CoGiveUpWithDefeatPrompt()
    {
        yield return StartCoroutine(CoShowCenterBattleMessagePrompt("戰敗", "您已放棄本局對戰", 0.9f));
        if (battleManager != null && !battleManager.IsBattleOver())
            battleManager.ForcePlayerSurrender();
    }
    ...
    private IEnumerator CoShowGiveUpDefeatPromptThenExit()
    {
        yield return StartCoroutine(CoShowCenterBattleMessagePrompt("戰敗", "您已放棄本局對戰", 1.1f));
