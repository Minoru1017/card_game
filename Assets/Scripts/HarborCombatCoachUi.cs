...
/// <summary>港灣訓練場實戰：林可姐戰術教練（關鍵時刻提示；見 HARBOR_COMBAT_COACH_GDD）。</summary>
public sealed class HarborCombatCoachUi : MonoBehaviour
{
    ...
    private HarborCombatCoachAdvisorSession _advisorSession;
    private HarborCombatHandHighlightMode _activeHighlightMode = HarborCombatHandHighlightMode.None;
    private int _boundBattleSessionId = -1;
    ...
    public static bool ShouldAllowHandHighlight()
    {
        if (!IsActiveForCurrentBattle) return false;
        if (!HarborCombatCoachPrefs.IsHandHighlightEnabled()) return false;
        return HarborCombatCoachAdvisor.ResolveHarborTier() != BattleDifficultyTier.Hard;
    }
        ...
        BattleSimulationDebugUI battleUi = null)
    {
        ...
        _advisorSession ??= new HarborCombatCoachAdvisorSession();
        _advisorSession.ResetForNewBattle(manager != null ? manager.GetHeroStartHealth() : 0);
    ...
    private void Update()
    {
        ...
        int sessionId = _manager.GetBattleSessionId();
        if (sessionId != _boundBattleSessionId)
        {
            _boundBattleSessionId = sessionId;
            _advisorSession ??= new HarborCombatCoachAdvisorSession();
            _advisorSession.ResetForNewBattle(_manager.GetHeroStartHealth());
        ...
        if (isPlayerTurn != _wasPlayerTurn)
        {
            ...
            if (isPlayerTurn)
            {
                _advisorSession?.ClearTurnWindow();
                _currentHintKey = string.Empty;
                ScheduleEvaluate(0f);
            }
            else
    ...
    private void SyncDiscardPhasePanelLayout()
    {
        ...
        else if (_currentHintKey == "discard_required" || _currentHintKey == "discard")
        {
            ClearHandPlayHighlights();
            _currentHintKey = string.Empty;
            _hasUnreadHint = false;
    ...
    private void PresentDiscardPhaseGuidance()
    {
        ...
        _advisorSession?.RemoveTurnWindowKey("discard_required");
        ClearHandPlayHighlights();
        if (_currentHintKey != "discard_required")
        {
            _currentHintKey = string.Empty;
            _hasUnreadHint = false;
        }
    ...
    {
        if (!IsActiveForCurrentBattle || !HarborCombatCoachPrefs.AreTacticalHintsEnabled())
        {
            ClearHandPlayHighlights();
    ...
    private void EvaluatePlayerTurnHints()
    {
        ...
        if (!ShouldRun() || _manager == null) return;
        if (!HarborCombatCoachPrefs.AreTacticalHintsEnabled()) return;
        if (!_manager.IsPlayerTurn() || _manager.IsBattleOver()) return;
        if (_manager.IsOpeningPresentationInProgress()) return;
        if (_manager.IsTurnSequenceInProgress() || _manager.IsSpellCastPresentationActive()) return;

        _advisorSession ??= new HarborCombatCoachAdvisorSession();
        if (!HarborCombatCoachAdvisor.TryEvaluate(_manager, _advisorSession, out HarborCombatCoachHint hint))
        {
    ...
    private void ShowHint(HarborCombatCoachHint hint)
    {
        if (hint == null || string.IsNullOrWhiteSpace(hint.RichMessage)) return;

        string key = hint.Key ?? string.Empty;
        if (IsDiscardPhaseActive())
        {
            if (key != "discard_required") return;
        }
        else if (key == "discard_required")
        {
            return;
        }
        ...
        string message = hint.RichMessage;
        ...
        _hasUnreadHint = true;
        _activeHighlightMode = hint.HighlightMode;

        HarborCombatCoachExpressionCatalog.ApplyToPortrait(_portraitImage, key);
        ...
        UpdateHandPlayHighlightsForActiveHint();
    }

    private void UpdateHandPlayHighlightsForActiveHint()
    {
        if (_battleUi == null && _manager != null)
            _battleUi = _manager.GetComponent<BattleSimulationDebugUI>();

        switch (_activeHighlightMode)
        {
            case HarborCombatHandHighlightMode.DiscardRecommend:
                _battleUi?.RequestHarborHandDiscardHighlights(_currentHintKey);
                break;
            case HarborCombatHandHighlightMode.PlayRecommend:
            case HarborCombatHandHighlightMode.LethalResponse:
                _battleUi?.RequestHarborHandPlayHighlights(_currentHintKey);
                break;
            default:
                ClearHandPlayHighlights();
                break;
    ...
    {
        _activeHighlightMode = HarborCombatHandHighlightMode.None;
        if (_battleUi == null && _manager != null)
            _battleUi = _manager.GetComponent<BattleSimulationDebugUI>();
        _battleUi?.ClearHarborHandPlayHighlights();
    }
    ...
    private void EnsureUi()
    {
        ...
        _root = new GameObject("HarborCombatCoach", typeof(RectTransform));
    ...
    {
        HarborCombatCoachExpressionCatalog.ApplyToPortrait(_portraitImage, _currentHintKey);
    }
    ...
    {
        GameObject backdrop = new GameObject("HarborCombatCoachBackdrop", typeof(RectTransform), typeof(Image));
    ...
    private TMP_FontAsset ResolveCoachFont()
    {
        ...
        Debug.LogWarning("HarborCombatCoachUi: 找不到支援中文的 TMP 字型，戰術提示可能無法顯示中文。");
