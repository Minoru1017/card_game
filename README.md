...
- 進入 **大廳**後作為主要落點，可由此前往其他功能場景（商店、牌組、自由對戰等，依場景配置為準）。

### 牌組管理（Deck Pack）

- 大廳 **「牌組」** 進入 **Deck Pack** 場景；點選五個牌組槽之一會有選中效果，並彈出 **查看牌組**／**編輯牌組**。
- **查看牌組**：進入背包（Persistent），畫面**只顯示該槽牌組內的卡牌**；若該槽為空則提示「該牌組為空」，不進入背包。
- **編輯牌組**：進入 Buildbeck 並**聚焦該槽**；此路徑下**不顯示**「準備好了／準備完成」進戰按鈕（純編輯）。

### 自由對戰（Free Battle）

- 大廳 **「自由對戰」** 進入 **Free Battle** 場景，可選 **綜合型／快攻型／防禦型** AI 陪練。
- 選定後進 **Buildbeck** 組牌，按 **「準備完成」** 開啟難度預覽；開戰前約 **70%** 機率觸發 **鬥鳥暖身** 隨機事件（可選參加或略過），再進入正式對戰。

### 全域導航（畫面上的「≡」）

多數非登入、非對戰中場景可開啟**左上選單**（含 **Deck Pack**、**Free Battle**、大廳、商店等 hub），通常包含：
